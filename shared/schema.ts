...
// ============================================================================
// PAYROLL CYCLES (Monthly run — parent of many payroll_ledger rows)
// ============================================================================
//
// One row per (store, year, month). Generated by cron on the 2nd of
// every month for the preceding month, or manually via the "Generate"
// button. Its child ledger rows carry the per-employee math.
//
// Status ladder:
//   pending  → generated but not yet approved (all fields editable)
//   approved → admin clicked "Approve payroll" → child ledgers locked,
//              RazorpayX push attempted per employee
//
// totalPayout and employeeCount are denormalised from the ledger for
// cheap dashboard rendering; a re-save of any child ledger recomputes
// them.

export const PAYROLL_CYCLE_STATUSES = ["pending", "approved"] as const;
...
export const payrollCycles = pgTable(
  "payroll_cycles",
  {
    ...
    status: text("status").notNull().default("pending").$type<PayrollCycleStatus>(),
