...
interface DelhiveryShipmentPayload {
  name: string;
  add: string;
  // Per the Shipment Creation schema, pin is an INTEGER. Typed as number|string
  // so the existing forward path (string) still compiles; reverse sends number.
  pin: number | string;
  city: string;
  state: string;
  country: string;
...
export class DelhiveryClient implements CourierProvider {
  ...
  async scheduleReversePickup(req: ReversePickupRequest): Promise<ReversePickupResult> {
    try {
      ...
      // Schema validation — enforce the Shipment Creation required set
      // (name, order, phone, add, pin, payment_mode + pickup_location.name)
      // and the 6-digit integer pin format BEFORE hitting the API.
      const addr = [req.pickupAddressLine1, req.pickupAddressLine2]
        .filter(Boolean)
        .join(', ')
        .trim();
      const pin = (req.pickupPincode ?? '').toString().trim();
      const missing: string[] = [];
      if (!req.customerName?.trim()) missing.push('name');
      if (!req.rmaNumber?.trim()) missing.push('order');
      if (!req.customerPhone?.trim()) missing.push('phone');
      if (!addr) missing.push('add');
      if (missing.length > 0) {
        return {
          success: false,
          error: `Missing required Delhivery shipment fields: ${missing.join(', ')}.`,
      ...
      if (!/^[1-9]\d{5}$/.test(pin)) {
        return {
          success: false,
          error: `Invalid pickup pincode "${pin}" — Delhivery requires a 6-digit Indian pincode.`,
          errorCode: 'INVALID_ADDRESS',
        };
      }

      // Serviceability pre-check — Delhivery MANDATES checking the Pincode
      // Serviceability API before order creation. Documented response shape:
      // delivery_codes empty → Non-Serviceable Zone; postal_code[].remark
      // "Embargo" → temporary NSZ; "" → serviceable. (The old code read a
      // non-existent `pickup` flag, which is why it never blocked.)
      const serv = await this.checkPincodeServiceability(pin);
      if (serv.serviceable === false) {
        return {
          success: false,
          error: serv.embargo
            ? `Pincode ${pin} is temporarily non-serviceable (Embargo) by Delhivery; a reverse pickup cannot be scheduled right now.`
            : `Pincode ${pin} is not serviceable by Delhivery (empty delivery_codes / Non-Serviceable Zone). A reverse pickup cannot be scheduled here. Raw: ${JSON.stringify(serv.raw)}`,
      ...
      const shipmentPayload: DelhiveryShipmentPayload = {
        ...
        order: req.rmaNumber,
        // "Pickup" payment mode = reverse leg (PUR is auto-scheduled by Delhivery).
        payment_mode: 'Pickup',
        ...
        // facility name registered in the Delhivery panel. return_* fields are
        // OPTIONAL per the schema; omitting them delivers to this warehouse.
      ...
      // Delhivery rejected the shipment — surface the EXACT raw JSON response so
      // ops sees precisely what Delhivery returned (no generic wrapper).
      const reason =
        response.data.packages?.[0]?.remarks?.join(', ') ||
        response.data.rmk ||
        response.data.error ||
        'Delhivery rejected the reverse pickup';
      const error = `${reason} [pickup_location="${pickupLocationName}", pin=${pin}] | Delhivery raw response: ${JSON.stringify(response.data)}`;
      console.error('[delhivery] reverse pickup rejected — raw response:', JSON.stringify(response.data));
      return { success: false, error, errorCode: classifyReverseError(reason) };
    } catch (error: any) {
      // Network / 5xx / timeout — include the raw error body Delhivery sent.
      const raw = error.response?.data;
      const msg =
        (raw && (raw.error || (typeof raw === 'string' ? raw : JSON.stringify(raw)))) ||
        error.message ||
        'Failed to reach Delhivery to schedule the reverse pickup';
      console.error('Delhivery scheduleReversePickup error:', raw || error.message);
      return { success: false, error: msg, errorCode: 'PROVIDER_UNAVAILABLE' };
    }
  }

  /**
   * Query Delhivery's Pincode Serviceability API and interpret it per the
   * official schema: `delivery_codes` empty → Non-Serviceable Zone;
   * postal_code[].remark "Embargo" → temporary NSZ; "" → serviceable.
   * Best-effort: on any error / inconclusive response returns
   * `serviceable: null` so callers never false-block a valid pickup.
   *
   * GET /c/api/pin-codes/json/?filter_codes=<pin>
   */
  async checkPincodeServiceability(
    pin: string,
  ): Promise<{ serviceable: boolean | null; embargo: boolean; raw?: any }> {
    try {
      const response = await this.client.get(
        `/c/api/pin-codes/json/?filter_codes=${encodeURIComponent(pin)}`,
      );
      const codes = (response.data as any)?.delivery_codes;
      if (!Array.isArray(codes)) {
        return { serviceable: null, embargo: false, raw: response.data };
      }
      if (codes.length === 0) {
        // Empty array = Non-Serviceable Zone.
        return { serviceable: false, embargo: false, raw: response.data };
      }
      const pcEntry = codes[0]?.postal_code;
      const pc = Array.isArray(pcEntry) ? pcEntry[0] : pcEntry;
      const remark = (pc?.remark ?? '').toString().trim().toLowerCase();
      if (remark === 'embargo') {
        return { serviceable: false, embargo: true, raw: response.data };
      }
      return { serviceable: true, embargo: false, raw: response.data };
    } catch (error: any) {
      console.warn(
        'Delhivery checkPincodeServiceability error:',
        error.response?.data || error.message,
      );
      return { serviceable: null, embargo: false };
    }
  }
